...
export default function SiswaDashboard({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "simulation" | "portfolio" | "mentor" | "leaderboard"
  >("overview");
  const [leaderboardFilter, setLeaderboardFilter] = useState<
    "global" | "logic" | "data-entry" | "package" | "validation"
  >("global");
  ...
  const logicSessions = studentSessions.filter(
    (s) =>
      s.gameName.toLowerCase().includes("logic") || s.gameId === "game-logic",
  );
  const entrySessions = studentSessions.filter(
    (s) =>
      s.gameName.toLowerCase().includes("data entry") ||
      s.gameId === "game-data-entry" ||
      s.gameName.toLowerCase().includes("administrasi"),
  );
  const packageSessions = studentSessions.filter(
    (s) =>
      s.gameName.toLowerCase().includes("package") ||
      s.gameId === "game-package" ||
      s.gameName.toLowerCase().includes("gudang") ||
      s.gameName.toLowerCase().includes("sort"),
  );
  ...
  const logicScore =
    logicSessions.length > 0
      ? Math.round(
          logicSessions.reduce((acc, s) => acc + s.score, 0) /
            logicSessions.length,
        )
      : student.skills.includes("Logika Pemrograman")
        ? 90
        : 75;
  ...
  const accuracyScore =
    entrySessions.length > 0
      ? Math.round(
          entrySessions.reduce((acc, s) => acc + s.metrics.accuracy, 0) /
            entrySessions.length,
        )
      : student.skills.includes("Data Entry")
        ? 85
        : 70;
  ...
  const motoricScore =
    packageSessions.length > 0
      ? Math.round(
          packageSessions.reduce((acc, s) => acc + s.metrics.accuracy, 0) /
            packageSessions.length,
        )
      : student.skills.includes("Penyortiran Barang") ||
          student.skills.includes("Perakitan Komponen")
        ? 80
        : 65;
  ...
  const consistencyScore =
    studentSessions.length > 0
      ? Math.round(
          studentSessions.reduce((acc, s) => acc + s.metrics.consistency, 0) /
            ...
            Math.min(
              ...
              100 -
                studentSessions.reduce(
                  (acc, s) => acc + s.metrics.completionTime,
                  0,
                ) /
                  studentSessions.length /
                  5,
  ...
  const leaderboardData = students.map((s) => {
    ...
    // Logic Quest
    const sLogicSessions = sSessions.filter(
      (gs) =>
        gs.gameId === "game-logic" ||
        gs.gameName.toLowerCase().includes("logic"),
    );
    const maxLogicScore =
      sLogicSessions.length > 0
        ? Math.max(...sLogicSessions.map((gs) => gs.score))
        : s.skills.includes("Logika Pemrograman")
          ? 90
          : 75;

    // Data Entry
    const sDataEntrySessions = sSessions.filter(
      (gs) =>
        gs.gameId === "game-data-entry" ||
        gs.gameName.toLowerCase().includes("data entry") ||
        gs.gameName.toLowerCase().includes("administrasi"),
    );
    const maxDataEntryScore =
      sDataEntrySessions.length > 0
        ? Math.max(...sDataEntrySessions.map((gs) => gs.score))
        : s.skills.includes("Data Entry")
          ? 85
          : 70;

    // Package Sorter
    const sPackageSessions = sSessions.filter(
      (gs) =>
        gs.gameId === "game-package" ||
        gs.gameName.toLowerCase().includes("package") ||
        gs.gameName.toLowerCase().includes("sorter") ||
        gs.gameName.toLowerCase().includes("sort") ||
        gs.gameName.toLowerCase().includes("gudang"),
    );
    const maxPackageScore =
      sPackageSessions.length > 0
        ? Math.max(...sPackageSessions.map((gs) => gs.score))
        : s.skills.includes("Penyortiran Barang") ||
            s.skills.includes("Perakitan Komponen")
          ? 80
          : 65;

    // Industry Validations
    const validationCount = validations.filter(
      (v) => v.studentId === s.id,
    ).length;
  ...
  const sortedLeaderboard = [...leaderboardData].sort((a, b) => {
    ...
    if (leaderboardFilter === "validation") {
      if (b.validationCount !== a.validationCount)
        return b.validationCount - a.validationCount;
  ...
  const myRank =
    sortedLeaderboard.findIndex((item) => item.student.id === student.id) + 1;
  const myStats = leaderboardData.find(
    ...
    try {
      ...
      if (data.success) {
        setChatMessages((prev) => [
          ...prev,
          { role: "assistant", text: data.reply, id: Date.now().toString() },
        ]);
      } else {
        setChatMessages((prev) => [
          ...prev,
  ...
  const handleDownloadPDF = async () => {
    ...
    const studentValidations = validations.filter(
    ...
    doc.setFontSize(8.5);
    doc.text(
      "Sistem Transisi Kompetensi Inklusif Nasional - SyncVoca",
      38,
      y + 17,
    );
    ...
    const competencies = [
      {
        label: "Logika Sekuensial (Simulasi Logic Quest)",
        score: logicScore,
        desc: "Pemecahan masalah terstruktur, penalaran komparatif, dan penanganan instruksi sekuensial.",
      },
      {
        label: "Akurasi Administrasi (Simulasi Data Entry)",
        score: accuracyScore,
        desc: "Ketepatan entri data verbal/numerik, pengecekan kesalahan, dan administrasi perkantoran.",
      },
      {
        label: "Ketangkasan Motorik & Sortir (Simulasi Sorter Paket)",
        score: motoricScore,
        desc: "Keterampilan motorik halus, penyortiran logistik gudang, pelabelan barang, dan efisiensi fisik.",
      },
      {
        label: "Konsistensi Fokus Kerja (Daya Tahan Kognitif)",
        score: consistencyScore,
        desc: "Keberlanjutan tingkat fokus sepanjang sesi pengerjaan tugas berulang.",
      },
      {
        label: "Kecepatan & Ketekunan Kerja",
        score: speedScore,
        desc: "Rasio waktu penyelesaian simulasi terhadap tolok ukur standar kecepatan industri.",
      },
    ];
    ...
    if (studentValidations.length === 0) {
      ...
      doc.setTextColor(148, 163, 184);
      doc.text(
        "Belum ada sertifikasi validasi industri yang diterbitkan secara resmi.",
        15,
        y,
      );
      y += 12;
      ...
      studentValidations.forEach((val) => {
        ...
        doc.setTextColor(148, 163, 184);
        doc.text(
          `Diterbitkan: ${new Date(val.sealIssuedAt).toLocaleDateString("id-ID")}`,
          20,
          y + 10,
        );
        ...
        doc.setTextColor(79, 70, 229);
        doc.text(
          `Keterampilan Teruji: ${val.validatedSkills.join(", ")}`,
          20,
          y + 14,
        );
    ...
    doc.setTextColor(71, 85, 105);
    doc.text(
      "Pernyataan Privasi Keluarga & Siswa (Family Privacy Guardrail):",
      18,
      y + 5,
    );
    ...
    doc.setTextColor(100, 116, 139);
    const privacyText =
      "Sesuai dengan kode etik privasi siswa, rekam medis internal, catatan keluarga, dan nomor kontak langsung wali siswa dijaga kerahasiaannya secara ketat dan tidak dipublikasikan secara umum kepada instansi luar tanpa persetujuan khusus keluarga siswa.";
    ...
    doc.setTextColor(148, 163, 184);
    doc.text(
      "Dokumen ini diterbitkan secara sah dan digital melalui platform SyncVoca.",
      105,
      287,
      { align: "center" },
    );
  ...
  return (
    <div
      className={`space-y-6 ${preferences.dyslexiaFont ? "font-serif" : "font-sans"} print:bg-white print:p-0`}
    >
        ...
        <div className="fixed top-6 right-6 z-[9999] max-w-sm w-full p-4 rounded-2xl bg-indigo-950 border border-indigo-500/30 text-white shadow-2xl flex items-start gap-3 animate-bounce">
          ...
          <div className="space-y-1 text-xs">
            ...
            <p className="text-[9px] text-zinc-500 mt-1">
              Dapat diuji sebagai notifikasi sistem setelah Anda memberikan izin
              di tab browser/PWA Anda.
            </p>
          </div>
          <button
      ...
      {/* Header Info Banner */}
      <div
        className={`p-6 sm:p-8 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 print:hidden ${
          isHighContrast
            ? "bg-white text-black border-4 border-black"
            : "bg-[#111] border border-white/5 bento-glow-indigo"
        }`}
      >
        <div className="space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`w-2.5 h-2.5 rounded-full ${isOnline ? "bg-emerald-400" : "bg-amber-400"} animate-pulse`}
            />
          ...
          <h2
            className={`${headingClass} ${isHighContrast ? "text-black" : "text-white"}`}
          >
            Halo, {student.name}!
          </h2>
          <p
            className={`${textClass} ${isHighContrast ? "text-neutral-700" : "text-zinc-400"}`}
          >
            Ayo latih terus kemampuan kerjamu, kumpulkan skor, dan siapkan
            resume masa depanmu.
          </p>
        </div>
        ...
        <div className="flex w-full max-w-full flex-wrap gap-2.5 md:w-auto md:max-w-[44%] md:justify-end">
          <span
            className={`max-w-full whitespace-normal break-words text-left text-xs font-bold px-3 py-1.5 rounded-xl border ${
              isHighContrast
          ...
          <span
            className={`max-w-full whitespace-normal text-xs font-bold px-3 py-1.5 rounded-xl border ${
              isHighContrast
      ...
      <div
        className={`flex flex-wrap sm:flex-nowrap gap-2 p-1.5 rounded-2xl max-w-2xl print:hidden ${
          isHighContrast
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 print:hidden">
          ...
          <div className="col-span-1 md:col-span-2 space-y-6">
            <div
              className={`p-6 sm:p-8 rounded-3xl space-y-6 ${
                isHighContrast
                  ...
                  : "bg-[#111] border border-white/5 bento-glow-emerald"
              }`}
            >
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                <div>
                  <h3
                    className={`font-bold text-lg font-display flex items-center gap-2 ${isHighContrast ? "text-black" : "text-white"}`}
                  >
                    <TrendingUp className="w-5 h-5 text-indigo-400" />
                    Analisis Kesiapan Kerja Vokasional
                  </h3>
                  <p className="text-xs text-zinc-500 mt-0.5">
                    Pemetaan multi-dimensi kompetensi kerja berdasarkan simulasi
                    waktu nyata.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">
                ...
                <div className="lg:col-span-3 h-[300px] w-full flex items-center justify-center relative sm:h-[280px]">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      <PolarGrid
                        stroke={isHighContrast ? "#000000" : "#cfe1d2"}
                      />
                      <PolarAngleAxis
                        dataKey="subject"
                        tick={{
                ...
                <div className="lg:col-span-2 space-y-4">
                  <div>
                    <div className="flex justify-between text-[11px] font-bold mb-1">
                      <span
                        className={
                          isHighContrast ? "text-black" : "text-zinc-400"
                        }
                      >
                        Logika Sekuensial
                      </span>
                    ...
                    <div
                      className={`h-2 rounded-full overflow-hidden ${isHighContrast ? "bg-zinc-200" : "bg-zinc-950/50 border border-white/5"}`}
                    >
                      <div
                  ...
                  <div>
                    <div className="flex justify-between text-[11px] font-bold mb-1">
                      ...
                      <span
                        className={
                          isHighContrast ? "text-black" : "text-amber-400"
                        }
                      >
                        {motoricScore}% (
                        {motoricScore >= 90
                  ...
                  <div>
                    <div className="flex justify-between text-[11px] font-bold mb-1">
                      ...
                      <span
                        className={
                          isHighContrast ? "text-black" : "text-cyan-400"
                        }
                      >
                        {consistencyScore}% (
                        {consistencyScore >= 90
                  ...
                  <div>
                    <div className="flex justify-between text-[11px] font-bold mb-1">
                      ...
                      <span
                        className={
                          isHighContrast ? "text-black" : "text-rose-400"
                        }
                      >
                        {speedScore}% (
                        {speedScore >= 90
            ...
            >
              <div className="flex justify-between items-center">
                <h3
                  className={`font-bold font-display text-sm ${isHighContrast ? "text-black" : "text-white"}`}
                >
                  Riwayat Latihan & Skor Sesi
                </h3>
              ...
              {studentSessions.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-zinc-500 mt-2">
                    Belum ada aktivitas. Silakan coba menu Simulasi Kerja!
                  </p>
                </div>
              ) : (
                  ...
                  {studentSessions.map((session, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p
                            className={`font-bold text-xs ${isHighContrast ? "text-black" : "text-zinc-200"}`}
                          >
                            {session.gameName}
                          </p>
                      ...
                      <div className="text-right">
                        <span
                          className={`font-mono font-bold text-xs ${isHighContrast ? "text-black" : "text-indigo-400"}`}
                        >
                          {session.score} Poin
                        </span>
                        <p className="text-[10px] text-zinc-500">
                          Akurasi {session.metrics.accuracy}%
                        </p>
                      </div>
                    </div>
                  ))}
          ...
          <div className="col-span-1 space-y-6">
            ...
            >
              <h3
                className={`font-bold font-display text-sm flex items-center gap-1.5 ${isHighContrast ? "text-black" : "text-white"}`}
              >
            ...
            <div
              className={`p-6 rounded-3xl space-y-4 relative overflow-hidden ${
                isHighContrast
                  ? "bg-white border-4 border-black text-black"
                  : "bg-indigo-600 text-white bento-glow-indigo"
              }`}
            >
              <div
                className={`absolute right-2 top-2 text-[9px] uppercase font-bold px-3 py-1 rotate-6 ${
                  isHighContrast
                    ? "bg-black text-white"
              ...
              <h3 className="font-bold font-display text-sm">
                Butuh Peningkatan Skor?
              </h3>
              <p
                className={`text-xs leading-relaxed ${isHighContrast ? "text-neutral-700" : "text-indigo-100"}`}
              >
                Skor akurasi input data atau ketangkasan sortir paket Anda
                sangat berpengaruh ke rating asisten vokasi di mata Mitra
                Industri. Terus asah logika Anda!
              </p>
              <button
                onClick={() => setActiveTab("simulation")}
            ...
            >
              ...
              <p className="text-[11px] leading-relaxed text-zinc-400">
                Aktifkan sistem notifikasi untuk menerima pengingat harian
                melatih simulasi karier serta status validasi mitra industri
                (DUDI) Anda.
              </p>
      ...
      {activeTab === "portfolio" && (
        <div
          className={`rounded-3xl p-6 sm:p-10 max-w-3xl mx-auto shadow-2xl print:shadow-none print:border-none print:p-0 relative ${
            isHighContrast
              ? "bg-white border-4 border-black text-black"
        ...
        >
          ...
          <div className="text-center space-y-3 pb-6 border-b border-white/5 print:border-neutral-300">
            <div
              className={`mx-auto w-16 h-16 rounded-2xl flex items-center justify-center font-black text-2xl print:bg-slate-100 print:text-black ${
                isHighContrast
                  ? "bg-black text-white"
                  : "bg-indigo-600 text-white"
              }`}
            ...
            <h1
              className={`text-2xl font-bold font-display tracking-tight uppercase ${isHighContrast ? "text-black" : "text-white"}`}
            >
              Portofolio Kerja Vokasi Digital
            </h1>
            <p className="text-[10px] text-zinc-500 uppercase font-mono tracking-widest">
              Sistem Transisi Kompetensi Inklusif Nasional
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm py-6">
            <div className="space-y-3">
              <span className="text-xs font-bold text-zinc-500 block uppercase font-mono tracking-wider">
                Informasi Pelamar
              </span>
              <div className="grid grid-cols-3 gap-y-2.5 text-xs">
                <span className="text-zinc-500 col-span-1">Nama</span>
                <span
                  className={`font-bold col-span-2 ${isHighContrast ? "text-black" : "text-zinc-100"}`}
                >
                  : {student.name}
                </span>
          ...
          <div className="space-y-4 text-sm py-6">
            <div>
              <span className="text-xs font-bold text-zinc-500 block uppercase font-mono tracking-wider mb-2">
                Profil Ringkat
              </span>
              <p
                className={`text-xs leading-relaxed ${isHighContrast ? "text-neutral-800" : "text-zinc-300"}`}
              >
                {student.bio}
              </p>
            </div>
          ...
          <div className="space-y-4 py-6">
            <span className="text-xs font-bold text-zinc-500 block uppercase font-mono tracking-wider">
              Skor Kompetensi Teruji (Hasil Latihan)
            </span>
            {studentSessions.length === 0 ? (
              <p className="text-xs text-zinc-500 py-2">
                Belum ada skor simulasi terekam.
              </p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {studentSessions.slice(0, 4).map((session, i) => (
                  <div
                    key={i}
                    className={`p-4 rounded-2xl flex justify-between items-center text-xs border ${
                      isHighContrast
                        ? "bg-zinc-100 border-black text-black"
                  ...
                  >
                    <div>
                      <p
                        className={`font-bold ${isHighContrast ? "text-black" : "text-white"}`}
                      >
                        {session.gameName}
                      </p>
                      <p className="text-[10px] text-zinc-500 mt-1">
                        Akurasi {session.metrics.accuracy}% | Konsistensi{" "}
                        {session.metrics.consistency}%
                      </p>
                    </div>
                    <span
                      className={`font-mono font-bold text-sm ${isHighContrast ? "text-black" : "text-indigo-400"}`}
                    >
                      {session.score} Pts
                    </span>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs pt-6">
            {validations.filter((v) => v.studentId === student.id).length ===
            0 ? (
              <div
                className={`p-5 rounded-2xl border ${
                  isHighContrast
                    ? "bg-zinc-100 border-black text-black"
                    : "bg-[#161616] border-white/5 text-zinc-400"
                }`}
              >
                ...
                <p className="text-[11px] leading-relaxed text-zinc-500">
                  Siswa ini sedang mengumpulkan rekam jejak simulasi kognitif
                  dan vokasi. Sertifikat Validasi Industri akan otomatis terbit
                  saat mitra industri meninjau hasil evaluasi kerja obyektif
                  siswa.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {validations
                  ...
                  .map((val) => (
                    ...
                    >
                      <span
                        className={`absolute right-2 top-2 text-[8px] uppercase font-bold px-3 py-0.5 rotate-6 ${
                          isHighContrast
                            ? "bg-black text-white"
                            : "bg-indigo-600 text-white"
                        }`}
                      ...
                      <p className="text-[11px] leading-relaxed text-zinc-400 print:text-neutral-700">
                        Disahkan pada{" "}
                        {new Date(val.sealIssuedAt).toLocaleDateString(
                          "id-ID",
                          { day: "numeric", month: "long", year: "numeric" },
                        )}
                        . Keterampilan teruji:{" "}
                        <strong className="text-indigo-400">
                          {val.validatedSkills.join(", ")}
                        </strong>
                        .
                      </p>
            ...
            >
              <h4
                className={`font-bold flex items-center gap-1 mb-2 text-xs ${isHighContrast ? "text-black" : "text-zinc-200"}`}
              >
                <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                Pernyataan Privasi Keluarga
              </h4>
              <p className="text-[10px] leading-relaxed text-zinc-500 print:text-neutral-500">
                Sesuai dengan kode etik privasi siswa, rekam medis internal,
                catatan keluarga, dan nomor kontak langsung wali siswa dijaga
                kerahasiaannya dan tidak dipublikasikan secara umum kepada
                instansi luar tanpa persetujuan khusus.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 print:hidden">
          ...
          <div className="col-span-1 space-y-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {student.skills.map((s) => (
                      <span
                        key={s}
                        className="px-2 py-0.5 rounded-md bg-zinc-800 text-[10px] text-zinc-300"
                      >
                        {s}
                      </span>
                    ))}
            ...
            >
              ...
              <p className="text-[11px] leading-relaxed text-zinc-400">
                Mentor AI menggunakan teknologi kecerdasan buatan Gemini untuk
                memberikan panduan karir vokasi yang disesuaikan secara pribadi
                dengan potensi, Profil Dukungan ABK, dan hasil simulasi kerja
                Anda.
              </p>
            </div>
          </div>
          ...
          <div className="col-span-1 lg:col-span-3 flex flex-col h-[560px]">
            <div
              className={`flex flex-col flex-1 rounded-3xl overflow-hidden border ${
                isHighContrast
            ...
            >
              ...
              <div className="p-3 bg-zinc-950/20 border-t border-white/5">
                <span className="text-[10px] text-zinc-500 block mb-2 px-1">
      ...
      {activeTab === "leaderboard" && (
        <div className="space-y-6 print:hidden">
          ...
          <div
            className={`p-6 sm:p-8 rounded-3xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6 ${
              isHighContrast
                ? "bg-white border-4 border-black text-black"
                : "bg-gradient-to-r from-indigo-950/40 via-purple-950/40 to-zinc-950/40 border border-white/5 bento-glow-indigo"
            }`}
          >
            <div className="space-y-2">
              <span
              ...
              <p className="text-xs text-zinc-400 max-w-xl">
                Latih diri Anda secara obyektif melalui simulasi kognitif dan
                vokasional. Hubungkan bakat unik Anda dengan pengakuan langsung
                dari Dunia Usaha & Industri (DUDI).
              </p>
            </div>

            {/* Personalized Rank Indicator Card */}
            <div
              className={`p-5 rounded-2xl shrink-0 min-w-[240px] flex items-center gap-4 ${
                isHighContrast
                  ? "bg-zinc-100 border-2 border-black"
                  : "bg-zinc-900/90 border border-white/5"
              }`}
            >
              ...
              <div className="text-xs">
                ...
                <div className="font-black text-lg text-indigo-400">
                  # {myRank} dari {students.length} Siswa
                </div>
          ...
          <div className="flex flex-wrap gap-2">
            {[
              {
                id: "global",
                label: "Global (Kesiapan Kerja)",
                icon: Award,
                desc: "Peringkat gabungan skor kognitif, motorik, dan seal validasi industri",
              },
              {
                id: "logic",
                label: "Logic Quest (Logika)",
                icon: Key,
                desc: "Skor tertinggi simulasi kognitif penyusunan urutan instruksi",
              },
              {
                id: "data-entry",
                label: "Data Entry (Administrasi)",
                icon: Keyboard,
                desc: "Skor tertinggi simulasi administrasi & akurasi ketikan data",
              },
              {
                id: "package",
                label: "Package Sorter (Logistik)",
                icon: Package,
                desc: "Skor tertinggi simulasi ketangkasan motorik halus dan sortir barang",
              },
              {
                id: "validation",
                label: "Seal Validasi Industri",
                icon: Sparkles,
                desc: "Jumlah sertifikasi resmi yang disetujui langsung oleh Mitra DUDI",
              },
            ].map((f) => {
              const Icon = f.icon;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              >
                ...
                <div className="divide-y divide-white/5">
                  {sortedLeaderboard.map((item, index) => {
                    ...
                    let suffix = "%";
                    if (leaderboardFilter === "global")
                      activeScore = item.globalScore;
                    else if (leaderboardFilter === "logic")
                      activeScore = item.logicScore;
                    else if (leaderboardFilter === "data-entry")
                      activeScore = item.dataEntryScore;
                    else if (leaderboardFilter === "package")
                      activeScore = item.packageScore;
                    ...
                    return (
                      <div
                        ...
                        className={`grid grid-cols-12 gap-3 p-4 sm:p-5 items-center transition ${
                          isMe
                            ? isHighContrast
                              ? "bg-zinc-100 font-bold border-l-4 border-black"
                      ...
                      >
                        ...
                        <div className="col-span-2 sm:col-span-1 flex justify-center">
                          {rank === 1 ? (
                            ...
                            <span className="font-mono text-zinc-500 text-xs font-bold">
                              {rank}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="col-span-6 sm:col-span-5 space-y-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span
                              className={`font-semibold text-xs ${isMe ? "text-indigo-400" : "text-white"}`}
                            >
                              {item.student.name}
                            </span>
                            {isMe && (
                          ...
                          {item.validationCount === 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center gap-1">
                              ...
                              <span className="text-[8px] text-zinc-500">
                                {validations
                                  .find((v) => v.studentId === item.student.id)
                                  ?.companyName.split(" ")
                                  .slice(1)
                                  .join(" ") || ""}
                              </span>
            ...
            <div className="col-span-1 space-y-6">
              ...
              >
                ...
                <p className="text-[11px] leading-relaxed text-zinc-400">
                  Peringkat dihitung secara obyektif berdasarkan tingkat
                  kesiapan kerja vokasional Anda. Inilah 3 cara utama
                  meningkatkan poin:
                </p>

                <div className="space-y-3 pt-2 text-xs">
                  <div className="flex gap-2">
                    <div className="w-5 h-5 rounded-md bg-zinc-800 text-indigo-400 font-bold flex items-center justify-center text-[10px] shrink-0">
                      1
                    </div>
                    <div>
                      <h5 className="font-semibold text-zinc-300 text-[11px]">
                        Tingkatkan Akurasi Game
                      </h5>
                      <p className="text-[10px] text-zinc-500 leading-relaxed">
                        Selesaikan simulasi dengan akurasi &gt;90%. Akurasi
                        tinggi membuktikan ketelitian Anda pada industri.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <div>
                      <h5 className="font-semibold text-zinc-300 text-[11px]">
                        Dapatkan Seal Validasi
                      </h5>
                      <p className="text-[10px] text-zinc-500 leading-relaxed">
                        Bila hasil kerja dinilai luar biasa, Mitra DUDI akan
                        menerbitkan Seal Validasi Resmi. Setiap Seal memberikan{" "}
                        <strong className="text-emerald-400">
                          +5 poin kesiapan kerja
                        </strong>
                        .
                      </p>
                    </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-500 leading-relaxed">
                        Hindari kesalahan berturut-turut pada simulasi berulang
                        demi menjaga skor kestabilan fokus.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <p className="text-[11px] leading-relaxed text-zinc-400">
                  Persaingan sehat di SyncVoca memotivasi seluruh siswa
                  berkebutuhan khusus untuk membuktikan kemampuan mereka.
                  Perusahaan mitra (DUDI) memantau klasemen ini secara berkala
                  untuk mencari kandidat terbaik untuk program kerja inklusif &
                  magang.
                </p>
              </div>
            </div>
