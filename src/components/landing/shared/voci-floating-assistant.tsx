...
export function VociFloatingAssistant() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const destination =
        typeof payload.destination === "string" && isVociDestination(payload.destination)
          ? payload.destination
          : undefined;
  ...
  return (
    <div className="fixed bottom-3 right-3 z-40 sm:bottom-5 sm:right-5">
      <section
        ...
        inert={!isOpen}
        className={cn(
          "absolute bottom-0 right-0 flex max-h-[min(82dvh,610px)] w-[min(calc(100vw-1.5rem),340px)] origin-bottom-right flex-col overflow-hidden rounded-[22px] border border-[#CFECDC] bg-white shadow-[0_24px_70px_rgba(4,41,27,0.22)] transition duration-300 ease-out sm:max-h-[min(80dvh,630px)] sm:w-[370px] sm:rounded-[24px]",
          isOpen
      ...
      >
        ...
        <div className="grid shrink-0 grid-cols-2 gap-1 border-b border-[#E2ECE6] bg-white p-2">
          <button
            type="button"
            className={cn(
        ...
        {mode === "quick" ? (
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            ...
            >
              {messages.map((message) => {
                ...
                return (
                  <div
                    key={message.id}
                    className={cn(
                      "max-w-[92%] rounded-2xl px-3.5 py-2.5 text-sm font-semibold leading-6",
                      message.role === "assistant"
                        ? "rounded-tl-md bg-[#F1F8F4] text-[#24304B]"
                        : "ml-auto rounded-br-md bg-[#0A9B58] text-white",
                    )}
                  >
                    <p>{message.content}</p>
                    {destination ? (
              ...
              {isSending ? (
                <div className="w-fit rounded-2xl rounded-tl-md bg-[#F1F8F4] px-3.5 py-2.5 text-xs font-bold text-[#66758A]">
                  Voci sedang menyiapkan jawaban...
                </div>
              ) : null}
              <div ref={chatEndRef} />
            </div>
            ...
            >
              ...
              <div className="flex items-end gap-2 rounded-2xl border border-[#D8E9DE] bg-white p-1.5 focus-within:border-[#82CDA1] focus-within:ring-2 focus-within:ring-[#DDF4E7]">
                <textarea
                  ...
                  maxLength={400}
                  rows={1}
                  placeholder="Contoh: bagaimana bukti kerja dibuat?"
