  ...
  return (
    ...
    >
          ...
          <div className="flex flex-col justify-between gap-7">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                >
                  SyncVoca Journey mengubah misi simulasi adaptif menjadi
                  portofolio kompetensi, rencana pendampingan, dan Industry
                  Validation Seal tanpa membuka data sensitif siswa.
                </p>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {heroProofs.map((item) => (
                ...
                >
                  <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#12843a]">
                    {item.label}
                  </p>
                  <p className="mt-1 text-sm font-black leading-snug text-[#17351f]">
                    {item.value}
                  </p>
                </div>
              ))}
            ...
            <div className="relative w-full max-w-lg rounded-[2rem] border border-[#dbe7dd] bg-gradient-to-b from-white to-[#eef8f0] p-5 shadow-xl">
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="font-display text-xl font-black text-[#17351f]">
                      Nadia Saputri
                    </h2>
                    <p className="text-xs font-bold text-[#61746a]">
                      Administrasi digital & desain grafis
                    </p>
                  </div>
                </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  >
                    <p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#61746a]">
                      {label}
                    </p>
                    <p className="mt-1 font-display text-3xl font-black text-[#12843a]">
                      {value}
                    </p>
                  </div>
                ))}
              ...
              <div className="mt-4 rounded-2xl bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-black text-[#17351f]">
                      Portofolio kompetensi digital
                    </p>
                    <p className="mt-1 text-xs font-semibold leading-relaxed text-[#61746a]">
                      Akurasi data entry, ketahanan tugas, komunikasi tertulis,
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-[#dbe7dd] bg-[#f8faf7] p-4">
                  ...
                  <p className="text-xs font-bold leading-relaxed text-[#61746a]">
                    Skill, skor, portofolio, kebutuhan akomodasi kerja.
                  </p>
                </div>
                <div className="rounded-2xl border border-[#dbe7dd] bg-[#f8faf7] p-4">
                  <div className="mb-2 flex items-center gap-2 text-[#12843a]">
                  ...
                  <p className="text-xs font-bold leading-relaxed text-[#61746a]">
                    Catatan medis, keluarga, dan catatan sensitif pendamping.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section id="workflow" className="scroll-mt-24 space-y-5">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-2 font-display text-3xl font-black text-[#17351f]">
              Dari intake menjadi validasi industri
            </h2>
          </div>
          <p className="max-w-xl text-sm font-medium leading-relaxed text-[#61746a]">
            Nama tahap di landing sama dengan workspace demo, sehingga user
            tidak pindah konteks saat masuk sebagai siswa, guru, orang tua,
            DUDI, atau admin.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          {journeyStages.map((item, index) => {
            ...
            return (
              <div
                key={item.title}
                className="rounded-2xl border border-[#dbe7dd] bg-white p-5 shadow-sm"
              >
                <div className="flex items-center justify-between gap-3">
                  <div className={`rounded-2xl p-3 ${item.tone}`}>
                    <Icon className="h-5 w-5" />
                  </div>
                  <span className="font-mono text-xs font-black text-[#c4d7c9]">
                    0{index + 1}
                  </span>
                </div>
                <h3 className="mt-5 text-sm font-black text-[#17351f]">
                  {item.title}
                </h3>
                <p className="mt-1 text-[11px] font-black uppercase tracking-[0.12em] text-[#12843a]">
                  {item.output}
                </p>
                <p className="mt-2 text-xs font-medium leading-relaxed text-[#61746a]">
                  {item.desc}
                </p>
              </div>
            );
      ...
      >
        <div className={`rounded-[2rem] p-5 sm:p-6 ${surfaceClass}`}>
          <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 className="mt-2 font-display text-3xl font-black text-[#17351f]">
                Output yang ditunjukkan ke juri
              </h2>
            </div>
            <p className="max-w-sm text-xs font-medium leading-relaxed text-[#61746a]">
              Klaim inklusi dibuat konkret lewat artefak yang bisa dibuka,
              dibaca, dan divalidasi.
            </p>
          </div>
            ...
            {productProofs.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-4 text-[10px] font-black uppercase tracking-[0.14em] text-[#61746a]">
                    {item.metric}
                  </p>
                  <h3 className="mt-1 text-sm font-black text-[#17351f]">
                    {item.title}
                  </h3>
                  <p className="mt-2 text-xs font-medium leading-relaxed text-[#61746a]">
                    {item.desc}
        ...
        <div className="rounded-[2rem] border border-[#dbe7dd] bg-[#0b5d2a] p-5 text-white shadow-xl sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[#d9f99d]">
                BMC signal
              </p>
      ...
      >
        <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-2 font-display text-2xl font-black text-[#17351f]">
              Masuk dari sudut pandang pengguna
            </h2>
          </div>
          <p className="max-w-sm text-xs font-medium leading-relaxed text-[#61746a]">
            Setiap role masuk ke journey yang sama, tetapi prioritas aksi dan
            hak melihat datanya berbeda.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h3 className="mt-4 text-sm font-black text-[#17351f]">
                    {card.title}
                  </h3>
                  <p className="mt-2 text-xs font-medium leading-relaxed text-[#61746a]">
                    {card.desc}
      ...
      <footer className="border-t border-[#dbe7dd] pt-6 text-center text-xs font-semibold text-[#61746a]">
        <p>
          © 2026 SyncVoca Journey. Menghubungkan potensi, menembus batas,
          mewujudkan kemandirian ekonomi inklusif.
        </p>
      </footer>
    </div>
  );
