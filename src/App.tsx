...
const roleMeta: Record<UserRole, { label: string; short: string; icon: any; desc: string }> = {
  siswa: {
    label: 'Siswa ABK',
    short: 'S',
    icon: User,
...
const roleOrder: Array<UserRole | null> = [null, 'siswa', 'guru', 'orang_tua', 'dudi', 'admin'];
...
function ResetDemoButton({ isHighContrast }: { isHighContrast: boolean }) {
  ...
  const handleResetData = async () => {
    const shouldReset = await requestConfirm({
      ...
      message: 'Semua progres demo lokal akan dikembalikan ke data ABK bawaan SyncVoca. Data ini hanya tersimpan di browser perangkat ini.',
  ...
  return (
    <button
      ...
      className={`rounded-full p-2 transition ${
        isHighContrast ? 'text-black hover:bg-zinc-100' : 'text-[#61746a] hover:bg-[#fff7d6] hover:text-[#0b5d2a]'
      }`}
    >
...
export default function App() {
  ...
  const dudiCandidates = useMemo(
    () => createDudiCandidateProfiles(students, sessions, validations, consents, jobPostings),
  ...
  const handleAddValidation = (
    ...
    const candidate = dudiCandidates.find((item) => item.studentRef === val.studentId);
    const hasIndustryConsent = consent?.status === 'approved' && consent.scopes.includes('industry-validation');
    ...
    const target = currentStudents.find((student) => student.id === val.studentId);
    if (target) {
  ...
  const handleUpdateConsent = (
    ...
    const existingConsent = currentConsents.find((consent) => consent.studentId === studentId);
    const targetStudent = database.getStudents().find((student) => student.id === studentId);
    const candidate = dudiCandidates.find((item) => item.studentRef === studentId);
    const now = new Date().toISOString();
    const fullScopes = ['portfolio-sharing', 'work-accommodation-sharing', 'industry-validation'];
    ...
    const nextConsent: StudentConsent = {
      ...
      studentId,
      status,
      scopes: status === 'approved' ? fullScopes : status === 'pending' ? pendingScopes : [],
      requestedBy: existingConsent?.requestedBy || targetStudent?.schoolName || 'Sekolah mitra SyncVoca',
      approvedBy: status === 'approved' ? (actorRole === 'admin' ? 'Admin SyncVoca' : 'Guru Pendamping SyncVoca') : undefined,
      updatedAt: now,
    ...
    const nextConsents = existingConsent
      ? currentConsents.map((consent) => consent.id === existingConsent.id ? nextConsent : consent)
  ...
  const handleDudiReportExport = (candidateId: string, fileName: string) => {
    const candidate = dudiCandidates.find((item) => item.candidateId === candidateId);
    database.addAuditEvent({
      actorRole: 'dudi',
  ...
  const handleAddPlacement = (
    ...
    const consent = consents.find((item) => item.studentId === placement.studentId);
    const candidate = dudiCandidates.find((item) => item.studentRef === placement.studentId);
    const hasIndustryConsent = consent?.status === 'approved' && consent.scopes.includes('industry-validation');
    ...
    if (actorRole === 'dudi' && !hasIndustryConsent) {
      ...
      return {
        ok: false,
        reason: 'consent_required',
        message: 'Consent validasi industri belum aktif untuk membuka pipeline placement.'
      };
    }

    const existingPlacement = placements.find(
      (item) => item.studentId === placement.studentId && item.jobId === placement.jobId
    );
  ...
  return (
    ...
    >
      ...
      <main className="mx-auto max-w-7xl px-3 py-5 sm:px-8 sm:py-8">
        {!currentRole ? (
          <LandingPage onEnterPortal={handleRoleChange} preferences={accessibility} />
        ) : (
          <JourneyWorkspace
            role={currentRole}
          ...
          return (
            <button
              ...
              className={`flex h-9 min-w-9 items-center justify-center rounded-full px-2 text-xs font-black transition sm:min-w-10 ${
                active
                  ? (isHighContrast ? 'bg-black text-white' : 'bg-[#12843a] text-white')
                  : (isHighContrast ? 'text-black hover:bg-zinc-100' : 'text-[#61746a] hover:bg-[#eef8f0] hover:text-[#0b5d2a]')
              }`}
